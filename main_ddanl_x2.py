...
class VIDEOSR(object):
    def __init__(self):
        ...
        self.beta1 = 0.9
        self.decay_step = 1e5
        self.train_dir = './DDANL_x2'

        self.pathlist = open('./data1/train/filelist_train.txt', 'rt').read().splitlines()
        random.shuffle(self.pathlist)
        self.vallist = open('./data1/eval/filelist_val.txt', 'rt').read().splitlines()
    ...
    def forward(self, frames_lr, is_training=True, reuse=False):
        ...
        with tf.variable_scope('LSTM'):
            cell = []
            cell.append(BasicConvLSTMCell.BasicConvLSTMCell([out_height // 2, out_width // 2], [3, 3], 16))
            cell.append(BasicConvLSTMCell.BasicConvLSTMCell([out_height // 2, out_width // 2], [3, 3], 32))
            cell.append(BasicConvLSTMCell.BasicConvLSTMCell([out_height // 2, out_width // 2], [3, 3], 64))
            rnn_state = []
            for i in range(7):
                rs = []
                for j in range(3):
    ...
    def evaluation(self):
        ...
        # write to log file
        with open('./DDANL_x2/eval_log_DDANLx2.txt', 'a+') as f:
...
def main(_):
    model = VIDEOSR()
    #model.train()
    model.testvideos('./image/LR/BSDS100',0,'ddan_lx2')
